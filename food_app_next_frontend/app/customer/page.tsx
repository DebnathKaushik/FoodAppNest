...
interface Restaurant {
  ...
  city: string;
  phone: string;
  image?: string; // optional restaurant image
}
...
export default function CustomerDashboard() {
  ...
  // Fetch restaurants
    ...
    const fetchRestaurants = async () => {
        try {
        const res = await axios.get("http://localhost:7000/Restaurant/all-restaurant"); // public API
        setRestaurants(res.data); // assume backend returns array of restaurants
        } catch (err) {
        console.error(err);
        } finally {
        setLoading(false);
  ...
  return (
        ...
        <div className="min-h-screen bg-gradient-to-tr from-sky-800 via-sky-600 to-cyan-400 backdrop-blur-sm">
